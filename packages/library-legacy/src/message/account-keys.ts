...
import {MessageCompiledInstruction,XMessageCompiledInstruction} from './index';
...
export class XMessageAccountKeys {
  ...
  compileInstructions(
    ...
    return instructions.map((instruction): XMessageCompiledInstruction => {
      return {
        programIdIndex: findKeyIndex(instruction.programId),
        accountKeyIndexes: instruction.keys.map(meta =>
          findKeyIndex(meta.pubkey),
        ),
        xAccountKeyIndexes: instruction.xKeys.map(meta =>
          findKeyIndex(meta.pubkey),
        ),
        data: instruction.data,
      };
